...
public class BFSCrawler {
   ...
   // private StrMatcher[] strMatchers;
     ...
     * @param seedURL
     */
    public void bfsCrawl(String seedURL) {
        ...
        Task<Void> task = new Task<Void>()
        {
            ...
            public Void call() throws Exception {
                ...
                toCrawl.add(normalizeURL(seedURL));
                ...
                if (!pageParser.canCrawl(seedURL)) {
                    ...
                    while (toCrawl.size() != 0 && crawled.size() < maxURLCrawled) {
                        ...
                        if(URLVerifier.verifyURL(url)) {
                            if (pageParser.canCrawl(url)) {
                                crawled.add(url);   //add it to the crawled set
                                String input = pageParser.getPageText(url).toLowerCase();
                                ...
                                for (String link : linksOnCurrPage) {
                                    ...
                                    if (!crawled.contains(link) && !toCrawl.contains(link)) {
